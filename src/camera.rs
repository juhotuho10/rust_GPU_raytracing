...
use glam::{mat4, quat, vec2, vec3a, vec4, Mat4, Quat, Vec2, Vec3A};
...
impl Camera {
    ...
    pub fn on_update(&mut self, timestep: &f32, egui_context: &Context) -> bool {
        ...
        if mouse_delta.x != 0.0 || mouse_delta.y != 0.0 {
            ...
            let q = (right_rotation * up_rotation).normalize();
    ...
    fn recalculate_projection(&mut self) {
        let fov_rad = self.vertical_fov.to_radians();
    ...
    fn recalculate_ray_directions(&mut self) {
        ...
        for y in 0..self.viewport_height {
            let y_coord = (y / self.viewport_height) as f32;
            for x in 0..self.viewport_width {
                let x_coord = (x / self.viewport_width) as f32;

                // normalized between -1 and 1
                let normalized_coord = vec2(x_coord, y_coord) * 2.0 - Vec2::ONE;

                let target = self.inverse_projection
                    * vec4(normalized_coord.x, normalized_coord.y, 1.0, 1.0);
                let world_space_target = self.inverse_view * target;

                let mut ray_direction: Vec3A = world_space_target.truncate().into();
                ray_direction = (ray_direction / world_space_target.w).normalize();
